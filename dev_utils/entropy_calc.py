...
def calc_ref_entropy(sample_list, entropy_file, rerun_ref):
    if rerun_ref:
...
def entropy_cluster(working_dir, ent_df, rerun_ref):
    ...
    return umap_df, ent_umap_df, piv_df, umap_fit, clusterer
...
def plot_renyi_entropy(working_dir, ent_df):
...
def plot_ent_clust(working_dir, ent_umap_df, umap_df, cpal):
    sns.set(rc={'figure.figsize': (12, 8)})
    sns.set_style("white")
    mark_list = ['.', 'o', 'v', '^', '<', '>', '8', 's', 'p', 'P', '*', 'H', 'X', 'D', 'd']
...
def calc_real_entropy(working_dir, mba_cov_list):
    ...
    ############################################################################################
    # Calculate entropy for all references
    ent_df = calc_ref_entropy(sample_list, entropy_file, rerun_ref)
    ent_results = entropy_cluster(ref_dir, ent_df, rerun_ref)
    umap_df = ent_results[0]
    ent_umap_df = ent_results[1]
    piv_df = ent_results[2]
    umap_fit = ent_results[3]
    clusterer = ent_results[4]

    if make_plots == True:
        cpal = plot_renyi_entropy(ref_dir, ent_df)
        plot_euc_dist(ref_dir, ent_df, cpal)
        plot_ent_clust(ref_dir, ent_umap_df, umap_df, cpal)
    ############################################################################################
    ...
    real_df = pd.read_csv(os.path.join(working_dir, 'entropy_table.tsv'), sep='\t', header=0)
