...
us_df['mode_rank'] = [mode2rank[x] for x in us_df['mode']]
us_df['param_rank'] = [param2rank[x] for x in us_df['param_set']]
us_df['mode_paramset'] = [str(x) + '_' + str(y) for x, y in
                          zip(us_df['mode'], us_df['param_set'])
                          ]
us_df['algo_param'] = ['_'.join([str(x), str(y)]) for x, y in
                       zip(us_df['algo'], us_df['param_set'])
                       ]
us_df['label_sample'] = ['_'.join([str(x), str(y)]) for x, y in
                         zip(us_df['label'], us_df['sample_id'])
                         ]
sm_abs_str_df = us_df.query("level == 'strain_absolute'")
sm_abs_str_df.sort_values(by=['type_rank', 'algo_rank',
                              'mode_rank', 'param_set'
                              ], inplace=True)
ss_abs_str_median_df = sm_abs_str_df.groupby(['mode', 'algo']
