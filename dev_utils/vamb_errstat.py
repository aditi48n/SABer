    ...
    for o_file in completed_files:
        ...
        for algo in err_df['algorithm'].unique():
            for level in err_df['level'].unique():
                ...
                ext_mq_uniq = ext_mq_df['exact_label'].unique().count()
                ext_nc_uniq = ext_nc_df['exact_label'].unique().count()
                str_mq_cnt = str_mq_df['MQ_bins'].count()
                str_nc_cnt = str_nc_df['NC_bins'].count()
                str_mq_uniq = str_mq_df['strain_label'].unique().count()
                str_nc_uniq = str_nc_df['strain_label'].unique().count()
