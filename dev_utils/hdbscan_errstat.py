...
#  TODO: fix the below file names so they read *_clean.tsv instead of denovo
denovo_clean_out_file = glob.glob(joinpath(saberout_path, 'clusters/*.denovo_denovo.tsv'))[0]
...
trusted_clean_out_file = glob.glob(joinpath(saberout_path, 'clusters/*.denovo_trusted.tsv'))[0]
...
ocsvm_clean_out_file = glob.glob(joinpath(saberout_path, 'clusters/*.denovo_ocsvm.tsv'))[0]
...
inter_clean_out_file = glob.glob(joinpath(saberout_path, 'clusters/*.denovo_inter.tsv'))[0]
...
# setup mapping to CAMI ref genomes
cluster_df = pd.read_csv(denovo_clean_out_file, sep='\t', header=0)
cluster_trim_df = cluster_df.query('best_label != -1')
src2contig_df = pd.read_csv(src2contig_file, header=0, sep='\t')
src2contig_df = src2contig_df.rename(columns={'@@SEQUENCEID': 'contig_id'})
contig_bp_df = src2contig_df[['contig_id', 'bp_cnt']]
...
score_tax_df.to_csv(denovo_clean_errstat_file, index=False, sep='\t')
...
# setup mapping to CAMI ref genomes
cluster_df = pd.read_csv(trusted_clean_out_file, sep='\t', header=0)
cluster_trim_df = cluster_df.query('best_label != -1')
src2contig_df = pd.read_csv(src2contig_file, header=0, sep='\t')
src2contig_df = src2contig_df.rename(columns={'@@SEQUENCEID': 'contig_id'})
contig_bp_df = src2contig_df[['contig_id', 'bp_cnt']]
...
score_tax_df.to_csv(trusted_clean_errstat_file, index=False, sep='\t')
