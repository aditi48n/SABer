...
gtdbtk_files = glob.glob(os.path.join(working_dir, "gtdbtk/gtdbtk.*.summary.tsv"))  # bac and arc gtdbtk results
...
keep_cols = ['SAG_ID', 'Bin Id', 'classification', 'Completeness', 'Contamination', 'Strain heterogeneity',
		'num_seqs', 'sum_len', 'min_len', 'avg_len', 'max_len', 'N50', 'qscore', 'pass_BARRNAP']
mq_df = sorted_df[keep_cols].query("Completeness >= @comp & Contamination <= @cont & qscore >= @qsmin & pass_BARRNAP == True").drop_duplicates('SAG_ID')
mq_df.to_csv(os.path.join(working_dir, 'dedupe/dedupe_mqhp.tsv'), index=False, sep='\t')

concat_top_list = []
for sag_id in list(mq_df['SAG_ID']):
	mq_comp = mq_df.query('SAG_ID == @sag_id')['Completeness'].values[0]
	mq_cont = mq_df.query('SAG_ID == @sag_id')['Contamination'].values[0]
	sag_tot_df = sorted_df.query("SAG_ID == @sag_id & Completeness == @mq_comp & Contamination == @mq_cont & pass_BARRNAP == True")
	concat_top_list.append(sag_tot_df)
